...
def main():
    ...
    # который лексический jaccard/containment пропускает (13-14.07).
    #
    # ПЕРЕКАЛИБРОВКА 28.07: абсолютный порог «5 общих основ на любой из 10 прошлых
    # реплик» бил по нормальному разбору одной темы. Boris задавал подряд десяток
    # вопросов про реферальную программу, и каждый мой ответ блокировался: он видел
    # первый текст, потом сжатый второй, то есть ровно тот дубль, против которого
    # сторож и стоит. Сторож сам производил дубли.
    # Теперь три условия вместо одного: сравниваем только с ближайшими ответами,
    # смотрим ДОЛЮ пересечения ядра, а не абсолютное число, и не трогаем короткие
    # реплики-дельты, которые Boris как раз и просит.
    CORE_BACK = 3          # только ближайшие ответы: тема живет дольше, перепев идет подряд
    CORE_MIN_LEN = 400     # короткая дельта не может быть «жеванием темы»
    CORE_MIN_SPEC = 8      # ядро меньше 8 основ не показатель
    CORE_RATIO = 0.5       # половина ядра финалки уже была в соседнем ответе

    def _spec(t: str):
        clean_t = re.sub(r"```[\s\S]*?```", "", t)
        return {w[:6] for w in re.findall(r"[a-zа-яё0-9]{6,}", clean_t.lower())}
    spec_f = _spec(final)
    if len(final) >= CORE_MIN_LEN and len(spec_f) >= CORE_MIN_SPEC:
        for prev in replies[:-1][-CORE_BACK:]:
            if len(spec_f & _spec(prev)) / len(spec_f) >= CORE_RATIO:
                print(json.dumps({"decision": "block", "reason": (
    ...
    # 28.07: короткую реплику containment не проверяем вообще. Дельта в две строки
    # физически состоит из уже прозвучавших слов, поэтому давала стопроцентное
    # совпадение и блокировалась — Boris получал вместо нее второй текст, то есть дубль.
    # Перепев-отчет, ради которого детектор и стоит, всегда длинный.
    CONTAIN_MIN_LEN = 300
    ratio_threshold = 0.52
    if len(final) >= CONTAIN_MIN_LEN and len(fw) >= CONTAIN_MIN_SIG:
