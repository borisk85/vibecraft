...
TASK_RE = re.compile(
    ...
    # короткие го-команды Boris (CLAUDE.md: «ОК/давай» = приказ к действию). «делай»
    # раньше НЕ ловилось (TASK_RE знал лишь «сделай/доделай»), команда «делай» =
    # выполни предложенное — терялась и не вставала в очередь (дыра 18.07).
    r"делай|давай|погнали|поехали|вперед|го"
    r")\b",
