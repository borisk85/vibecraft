...
CHECKS = [
    ...
    # Класс ошибки (23.07): паттерн ловил ЛЮБОЕ число со словом «минут» и глушил
    # все инструменты на цифрах из отчета аналитики, на настройке кулдауна и даже
    # на цитировании самого правила. Теперь блокируем только обещание времени
    # выполнения: рядом обязан стоять глагол ожидания или название процесса.
    (re.compile(
        r"(?:займ[её]т|займ[ую]т|потребуется|уйд[её]т|подожд\w*|ожида\w*|"
        r"это\s+будет\s+идти|отработает\s+за|выполнится\s+за|"
        r"билд|сборка|установка|деплой|прогон|скрипт|команда)"
        r"[^.\n]{0,45}?"
        r"(?:~\s*)?\b\d+(?:[.,]\d+)?\s*(?:[-–—]\s*\d+\s*)?"
        r"(?:сек\.?\b|секунд\w*|мин\.?\b|минут\w*)|"
        r"~\s*\d+(?:[.,]\d+)?\s*(?:[-–—]?\s*\d*)\s*(?:сек|секунд|мин|минут)\w*|"
        r"(?:займ[её]т|займ[ую]т|потребуется|уйд[её]т|идет|будет\s+идти)\s+"
        r"(?:около|порядка|примерно|пар[ауеы]|несколько)\s+(?:секунд|минут\w*)",
        re.IGNORECASE | re.UNICODE),
     False,
