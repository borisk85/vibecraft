...
TY_PRAV_RE = re.compile(
    ...
    r"\bправ\s*[,.!;:—–]|"
    # «прав»/«правы» как ОТДЕЛЬНОЕ слово-сказуемое: «Boris прав», «он прав в том»,
    # «тут ты прав», «вы правы». Старый рег требовал «ты» ПЕРЕД или знак ПОСЛЕ и
    # пропускал «<кто-то> прав <слово>» (дыра 18.07: «Boris прав в диагнозе»).
    # \bправ\b НЕ трогает правк/прав(ить)/поправк/правил/право/права/справа/направо —
    # там после «прав» идёт буква, границы слова нет.
    r"\bправ\b|\bправы\b|"
