...
TASK_CMD_RE = re.compile(
    ...
    r"съезжа\w*|налез\w*|перекры\w*|обрез\w*|торчит|вылез\w*|косо\w*|крив\w*|"
    r"битый|отвал\w*|пропал\w*|не\s+влезае\w*|разъеха\w*|поехал\w*)\b|"
    r"не\s+работает|\bбаг\b|не\s+видно",
