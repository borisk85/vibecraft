...
export async function PUT(
  ...
  try {
    ...
    if (updatedTs === blogTs) {
      // Статья выше уже найдена (extractArticle), значит find+parse сработали.
      // Равенство здесь означает ТОЛЬКО одно: новый текст побайтно совпал с тем,
      // что уже на ветке — сохранять нечего. Это НЕ ошибка патча. Раньше отдавали
      // 500 «Не получилось патчить файл», из-за чего повторное сохранение выглядело
      // как поломка и блокировало публикацию. Отдаем 200 «без изменений».
      return NextResponse.json(
        {
          ok: true,
          unchanged: true,
          message: "Изменений нет — на ветке уже актуальный текст, можно публиковать",
        },
        { status: 200 },
      );
    }
